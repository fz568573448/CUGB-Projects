<?php
    ...
    else
    {
        ...
        if ($a["verification"]==$_GET["verification"])
        {
            mysql_query("UPDATE `user` SET `flag`=1 WHERE `id`={$_GET['id']}");
            echo '<script type="text/javascript">alert("验证成功，你已完成报名！"); window.location.href="."</script>';
        }
