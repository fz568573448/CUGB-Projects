...
class cugbCourse {
    ...
    public function loadCourse() {
        $data = $this->getHtml('http://jwc.cugb.edu.cn/academic/accessModule.do?groupId=&moduleId=2000', "", $_SESSION["remote_cookie"]);
        return $data;
    }
    public function getWeek() {
        $data = $this->getHtml('http://jwc.cugb.edu.cn/academic/listLeft.do', "", "");
        if (preg_match('/第(\d*)周/',$data,$week)) return $week[1];
        else return 1;
    }
}
function updateCache()
{
    $myfileR = fopen("cache.appcache", "r") or die("Unable to open file!");
    fgets($myfileR);
    fscanf($myfileR,"#TIMES %d",$TIMES);
    $content=fread($myfileR,1000);
    fclose($myfileR);
    $myfileW = fopen("cache.appcache", "w") or die("Unable to open file!");
    fprintf($myfileW,"CACHE MANIFEST\r\n#TIMES %d\r\n%s",$TIMES+1,$content);
    fclose($myfileW);
}
?>
